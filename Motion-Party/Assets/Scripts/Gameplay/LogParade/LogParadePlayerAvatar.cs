...
public class LogParadePlayerAvatar : MonoBehaviour
{
    ...
    public void SetLaneInstant(int lane)
    {
        ...
        if (showDebugInfo)
            Debug.Log($"Avatar positionné instantanément sur la voie {lane}");
    }
    ...
    private void StartMovementToLane()
    {
        if (enableSmoothMovement)
        {
            ...
            if (showDebugInfo)
                Debug.Log($"Démarrage du mouvement vers la voie {targetLane}");
        }
        else
        {
    ...
    private void UpdateMovement()
    {
        ...
        if (movementProgress >= 1f)
        {
            ...
            if (showDebugInfo)
                Debug.Log($"Mouvement terminé. Avatar sur la voie {targetLane}");
        }
        else
        {
