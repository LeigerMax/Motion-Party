...
namespace Gameplay.LogParade.Core
{
    ...
    #region Calibration Process
        ...
        private IEnumerator LaunchGameAfterDelay()
        {
            ...
            if (gameLauncher != null)
            {
                ...
                LogParadeLogger.Log("Lancement du jeu via GameLauncher");
            }
            else
            {
                LogParadeLogger.LogError("Impossible de lancer le jeu : GameLauncher manquant.");
            }
        }
    ...
    #region Debug/ContextMenu
        ...
        public void ForceLaunchGame()
        {
            ...
            {
                gameLauncher.StartCompleteGameProcess();
                LogParadeLogger.Log("Lancement du jeu via GameLauncher (debug)");
            }
            else
            {
                LogParadeLogger.LogError("Impossible de lancer le jeu : GameLauncher manquant.");
