...
public class DebugPanel_Calibration : BaseDebugPanel
{
    ...
    private string currentCalibrationStep = "Non démarré";
    ...
    /// <summary>
    /// Dessine la section état de calibration
    /// </summary>
    private void DrawCalibrationStatusSection()
    {
        ...
        // Étape actuelle
        GUILayout.Label($"Étape: {currentCalibrationStep}");
    ...
    /// <summary>
    /// Dessine la section progression
    /// </summary>
    private void DrawProgressSection()
    {
        ...
        string progressBar = CreateProgressBar(calibrationProgress, 100);
        GUILayout.Label($"Progrès: {progressBar}");
        GUILayout.Label($"Valeur: {calibrationProgress}%");
        
        // Détails de progression si en cours
        if (isCalibrating)
        {
            GUILayout.Label("<color=yellow>Calibration en cours...</color>");
        }
        else if (isCalibrated)
        {
            GUILayout.Label("<color=green>Calibration terminée ✓</color>");
        }
    }
    ...
    private void AutoFindCalibrationComponents()
    {
        ...
        {
            calibrationInteractive = FindObjectOfType<LogParadeCalibrationInteractive>();
        }
    ...
    private void RefreshCalibrationStatus()
    {
        ...
        if (calibrationManager != null)
        {
            ...
            currentCalibrationStep = GetCurrentStep();
        }
    ...
    private bool GetIsCalibrated()
    {
        ...
        var possibleFields = new string[] { "isCalibrated", "_isCalibrated", "calibrated", "isCalibrationComplete" };
        ...
        var possibleProperties = new string[] { "IsCalibrated", "Calibrated", "IsCalibrationComplete" };
    ...
    {
        if (calibrationInteractive == null) return "Non disponible";
        
        var field = calibrationInteractive.GetType().GetField("currentStep", 
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        
        if (field != null)
        {
            var stepValue = field.GetValue(calibrationInteractive);
            return stepValue?.ToString() ?? "Inconnu";
        }
    ...
    private int GetCalibrationProgress()
    {
        if (calibrationInteractive == null) return 0;
        
        // Essayer différents champs possibles
        var progressField = calibrationInteractive.GetType().GetField("calibrationProgress", 
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        
        if (progressField != null)
        {
            return (int)progressField.GetValue(calibrationInteractive);
        }
        
        // Calcul alternatif basé sur l'état
        if (isCalibrated) return 100;
        if (isCalibrating) return 50;
        return 0;
    }
    ...
    private string CreateProgressBar(int progress, int max)
    {
        int barLength = 8;
        int filled = Mathf.RoundToInt((float)progress / max * barLength);
        string bar = "";
